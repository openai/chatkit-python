...
Only one client tool call is allowed per turn, and the agent must stop at the tool before continuing. See also [Call client-side tools during inference](add-features/call-client-side-tools-during-inference.md).
